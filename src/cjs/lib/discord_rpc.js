...
if (os.platform() === "win32")
  IPCTransport = require("./transports/ipc_windows");
else if (os.platform() === "darwin")
  IPCTransport = require("./transports/ipc_mac");
else IPCTransport = require("./transports/ipc_linux");

const { OPCODES } = require("./constants");
const { generate_nonce } = require("./helpers");
...
class DiscordRPC extends EventEmitter {
    ...
    try {
      ...
      this.transport.onData((op, payload) => {
        try {
          ...
          console.error("[rpc] handle_message error:", e && e.message);
        }
      });
      ...
      if (sock) {
        ...
        sock.on("error", (err) => {
          console.error("[rpc] socket error:", err && err.message);
        });
      }
      ...
      console.error("[rpc] connect error:", err && err.message);
    ...
    if (payload.cmd === "DISPATCH" && payload.evt === "READY") {
      ...
      q.forEach((fn) => {
        try {
          ...
          console.error("[rpc] queued fn error:", e && e.message);
        }
      });
  ...
  send(op, data) {
    if (!this.transport || !this.transport.socket) {
      if (op === OPCODES.FRAME) {
        this.queue.push(() => this.send(op, data));
      }
      return;
    }
